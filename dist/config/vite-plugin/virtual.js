...
export const configModule = {
    ...
    load: async (solidBaseConfig, _root) => `export const solidBaseConfig = ${JSON.stringify(solidBaseConfig)};`,
};
export const componentsModule = {
    ...
    async load(theme) {
        ...
        const mdxComponentFiles = [];
        for (let i = 0; i < themePaths.length; i++) {
            const themePath = themePaths[i];
            const dir = await readdir(themePath).catch(() => []);
...
function getMarkdownModulePath(id) {
    const [pathname, search = ""] = id.split(/\?(.*)/s);
    const params = new URLSearchParams(search);
    const nestedId = params.get("id");
    if (nestedId)
        return nestedId.split("?")[0];
    return pathname;
}
export async function transformMdxModule(code, id, solidBaseConfig) {
    ...
    if (solidBaseConfig.editPath && modulePath.startsWith(rootPath)) {
        ...
        if (typeof solidBaseConfig.editPath === "string")
            ...
            modulePathLink = solidBaseConfig.editPath(path);
    }
    let lastUpdated = 0;
