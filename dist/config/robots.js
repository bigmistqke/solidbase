...
function getRobotsSitemapUrl(config) {
    if (!config.robots)
        return undefined;
    if (config.robots === true)
        return getDefaultSitemapUrl(config);
    if (config.robots.sitemap === false)
        return undefined;
    if (typeof config.robots.sitemap === "string")
        return config.robots.sitemap;
    return getDefaultSitemapUrl(config);
}
