...
function chunkEntries(entries, maxUrlsPerSitemap) {
    const chunks = [];
...
export function serializeSitemapIndex(urls) {
    const body = urls
...
export function buildSitemapXmlFiles(hostname, entries, options = {}) {
    const maxUrlsPerSitemap = options.maxUrlsPerSitemap ?? DEFAULT_MAX_URLS_PER_SITEMAP;
    ...
    const sitemapIndex = serializeSitemapIndex(sitemapFiles.map(({ fileName }) => new URL(fileName, hostname).toString()));
