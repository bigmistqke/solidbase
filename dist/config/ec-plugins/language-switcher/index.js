import { definePlugin, ExpressiveCodeBlock, ExpressiveCodeEngine, MetaOptions, } from "@expressive-code/core";
import { addClassName, h, SKIP, visit, } from "expressive-code/hast";
import rangeParser from "parse-numeric-range";
import { MarkerTypeOrder, tsToJs, } from "./ts-to-js.js";
const SUPPORTED_LANGS = ["ts", "typescript", "tsx"];
function getMarkerType(input) {
...
function addToggleToHeader(header) {
 ...
 */
export function ecPluginLanguageSwitcher(options) {
    return definePlugin({
        ...
        hooks: {
            postprocessRenderedBlock: async ({ renderData, codeBlock, config }) => {
                ...
                const { jsCode, markers: jsMarkers } = await tsToJs(codeBlock.code, markers, isJsx, options.formatter);
