...
const tripleSlashDirectiveRegExp = /^\s*\/\/\/\s*<(reference|amd-module|amd-dependency)\b.*?\/?>[\s\S]*$/;
function removeTypeParameters(ms, node) {
...
function removeTripleSlashDirectives(ms, ast) {
...
function processImportDeclaration(ms, node) {
...
function processNamedBindings(ms, parentNode, namedBindings) {
...
function processNamedImports(ms, node) {
    const namedBindings = node.importClause.namedBindings;
    processNamedBindings(ms, node, namedBindings);
}
function processExportDeclaration(ms, node) {
    if (node.isTypeOnly) {
        ms.remove(node.getFullStart(), node.getEnd());
        return;
    }
...
function hasModifier(node, modifierKind) {
    return (node.modifiers?.some((mod) => mod.kind === modifierKind) || false);
}
function removeMethodSignatures(ms, node) {
...
function removeDeclareStatements(ms, node) {
    const isDeclareStatement = (ts.isVariableStatement(node) &&
...
function removeTypeAnnotation(ms, node) {
    if (!node.type) {
        return;
    }
    const children = node.getChildren();
    const questionToken = children.find((child) => child.kind === ts.SyntaxKind.QuestionToken);
    if (questionToken) {
        ms.remove(questionToken.getFullStart(), questionToken.getEnd());
    }
    const colonToken = children.find((child) => child.kind === ts.SyntaxKind.ColonToken);
    if (colonToken) {
        ms.remove(colonToken.getFullStart(), node.type.getEnd());
    }
}
function removeAccessModifiers(ms, node) {
...
function processFunctionLike(ms, node) {
...
function processPropertyDeclaration(ms, node) {
    removeAccessModifiers(ms, node);
    removeTypeAnnotation(ms, node);
    if (node.exclamationToken) {
        ms.remove(node.exclamationToken.getFullStart(), node.exclamationToken.getEnd());
    }
}
function processClassDeclaration(ms, node) {
    const modifiersToRemove = [
        ts.SyntaxKind.AbstractKeyword,
...
function transformNode(ms, node) {
    ...
    if (ts.isAsExpression(node)) {
        ms.overwrite(node.getFullStart(), node.getEnd(), node.expression.getFullText());
        return false;
    }
...
export const MarkerTypeOrder = ["mark", "del", "ins"];
export async function tsToJs(tsCode, tsMarkers, isJsx, formatter) {
    const fileName = isJsx ? "temp.tsx" : "temp.ts";
    const ast = ts.createSourceFile(fileName, tsCode, ts.ScriptTarget.Latest, true, isJsx ? ts.ScriptKind.TSX : ts.ScriptKind.TS);
    ...
    const rawJsMap = await new SourceMapConsumer(ms.generateMap({
        file: "temp.js",
        source: fileName,
    }));
    const formattedJsMap = await new SourceMapConsumer(formatMs.generateMap({
        file: "temp.js",
        source: "unformatted.js",
    }));
    const tsToJsMap = new Map();
    ...
    const unformattedToFormattedMap = new Map();
...
//# sourceMappingURL=ts-to-js.js.map