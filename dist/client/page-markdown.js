...
import { useCurrentPageData } from "./page-data.js";
const markdownCache = new Map();
const BUTTON_RESET_MS = 2500;
function isLlmsDisabled(llms) {
    return llms === false || llms?.exclude === true;
}
export function canCopyPageMarkdown(configLlms, llms) {
...
export function getCurrentPageMarkdownPath(pathname = globalThis.location?.pathname) {
    return pathname ? toMarkdownPath(pathname) : undefined;
}
export async function getCurrentPageMarkdown(pathname = globalThis.location?.pathname, fetchImpl = globalThis.fetch) {
...
export async function copyTextToClipboard(text, writeText = writeClipboard) {
...
export function useCopyPageMarkdown() {
    ...
    const canCopy = () => canCopyPageMarkdown(config().llms, pageData()?.frontmatter.llms);
    const isReady = () => isClient() && canCopy();
    async function copy() {
        if (!isReady() || isCopying())
            return false;
