...
import { getThemeVariant, setTheme, } from "../../client/index.jsx";
...
const THEME_OPTIONS = [
    ...
    },
    {
        ...
        label: "System",
        icon: () => (<div>
				<IconSunLine class={`${styles["system-light"]} ${styles["force-light"]}`} aria-hidden/>
				<IconMoonLine class={`${styles["system-dark"]} ${styles["force-dark"]}`} aria-hidden/>
			</div>),
    },
];
export default function ThemeSelector() {
    return (<Select class={styles.root} options={THEME_OPTIONS} optionValue="value" optionTextValue="label" value={THEME_OPTIONS.find((t) => t.value === getThemeVariant())} onChange={(option) => {
            setTheme(option?.value);
        }} allowDuplicateSelectionEvents gutter={8} sameWidth={false} placement="bottom" itemComponent={(props) => (<Select.Item class={styles.item} item={props.item}>
					<Select.ItemLabel>
			...
			<Select.Trigger class={styles.trigger} aria-label="Change theme mode">
				<Select.Value>
					{(state) => (<RefreshOnMount aria-label={state.selectedOption().label}>
...
function RefreshOnMount(props) {
    ...
    return (<Show when={refresh()} fallback={<div {...props}>{resolved()}</div>} keyed>
