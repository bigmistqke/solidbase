...
import { useCopyPageMarkdown, } from "../../client/index.jsx";
...
export default function CopyPageLink() {
    ...
    return (<Show when={canCopy()}>
			<button type="button" class={styles.button} classList={{
            [styles.success]: state() === "success",
            [styles.error]: state() === "error",
        }} onClick={copy} disabled={!isReady() || import.meta.env.DEV} aria-busy={isCopying() || undefined} aria-live="polite">
				...
				<span class={styles.labelWrap}>
					...
					<span class={styles.label} classList={{ [styles.active]: state() === "success" }}>
						{text.copiedPage}
					</span>
					<span class={styles.label} classList={{ [styles.active]: state() === "error" }}>
						{text.copyFailedPage}
					</span>
				</span>
			</button>
...
//# sourceMappingURL=CopyPageLink.jsx.map