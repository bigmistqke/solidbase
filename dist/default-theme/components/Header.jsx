...
export default function Header() {
    const [tocRef, setTocRef] = createSignal();
    const [navRef, setNavRef] = createSignal();
    const { ThemeSelector, LocaleSelector, VersionSelector, TableOfContents } = useDefaultThemeComponents();
    const { tocOpen, setTocOpen, setSidebarOpen, frontmatter, navOpen, setNavOpen, } = useDefaultThemeState();
    const config = useRouteConfig();
    const locale = useLocale();
    const sidebar = useSidebar();
    const tocContent = () => useCurrentPageData()()?.toc;
    const hasSidebar = () => frontmatter()?.sidebar !== false &&
        sidebar() &&
        sidebar().items.length > 0;
    const hasToc = () => frontmatter()?.toc !== false && tocContent() && tocContent().length > 0;
			...
			<div>
				...
				<div class={styles["top-nav"]}>
					<Dialog open={navOpen()} onOpenChange={setNavOpen} modal={false}>
						<Dialog.Trigger type="button" class={styles["mobile-nav-menu"]} aria-label="Open navigation">
			...
			<Show when={hasSidebar() || hasToc()}>
				<div class={styles["mobile-bar"]}>
					<Show when={hasSidebar()} fallback={<div />}>
						<button type="button" class={styles["mobile-menu"]} onClick={() => setSidebarOpen((p) => !p)} aria-label="Open navigation">
					...
					<Dialog open={tocOpen()} onOpenChange={setTocOpen} modal={false}>
						<Show when={hasToc()}>
							<Dialog.Trigger type="button" class={styles["mobile-menu"]} aria-label="Open table of contents">
								On this page <IconArrowDownLine />
							</Dialog.Trigger>
						</Show>

						<Dialog.Portal mount={tocRef()}>
							<Dialog.Content class={styles["toc-popup"]} onClick={() => setTocOpen(false)}>
								<TableOfContents />
							</Dialog.Content>
						</Dialog.Portal>
					</Dialog>
